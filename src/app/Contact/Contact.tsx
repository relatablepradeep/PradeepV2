...
import { Mail, Github, Linkedin, Twitter, ExternalLink, X } from "lucide-react";
...
export default function Contact() {
  ...
  const handleSocialClick = (social: any, socialIndex: number) => {
  ...
  const getVisibleSocials = () => {
    const result = [];
    for (let i = 0; i < 4; i++) {
      const socialIndex = (socialLinks.length - 1 - Math.floor(offset / 140) - i + socialLinks.length * 1000) % socialLinks.length;
      result.push({
    ...
    <div className="fixed inset-0  overflow-hidden">
      ...
      >
        <div
          ...
          style={{ 
            cursor: expandedSocial ? 'default' : (isDragging ? 'grabbing' : 'grab')
          }}
        ...
        >
          {visibleSocials.map((social, i) => {
            ...
            return (
              <div
                key={social.key}
                className={`absolute left-1/2 -translate-x-1/2 w-20 h-20 ${social.color} ${social.hoverColor} text-white rounded-full shadow-2xl cursor-pointer transition-all duration-300 hover:shadow-3xl hover:scale-110 active:scale-95 flex items-center justify-center group`}
                style={{
          ...
          >
            ...
            <div className="space-y-6 text-center">
              ...
              <div className="space-y-4">
                <div className={`w-20 h-20 ${expandedSocial.color} rounded-full flex items-center justify-center mx-auto shadow-xl`}>
                  <expandedSocial.icon size={40} className="text-white" />
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-gray-800 mb-1">{expandedSocial.name}</h2>
                  <p className="text-gray-500 font-mono text-sm">{expandedSocial.username}</p>
                </div>
              </div>
